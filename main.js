...
var VERSION=2.021;
var BETA=1;
var totalNumCookies = 0;
var totalTempCookies = 0;
...
window.addEventListener('load', () => {
	...
	document.getElementById("physicsEngine").style.pointerEvents = 'none';
	//Body.set(ClickedObject[0], "position", {x: x, y: y})
	document.getElementById("wrapper").onmousedown = (event) => {
		
		ClickedObject = Query.point(allBodies, {x: event.pageX, y: event.pageY})

		function mouseDownMoveCookie(event){
			moveCookie(event.pageX, event.pageY)
		}

		function moveCookie(x, y){
			console.log(x, y)
			console.log(ClickedObject[0])
			Body.set(ClickedObject[0], "position", {x: x, y: y})
			Body.setStatic(ClickedObject[0], true)
			
		}

		if (ClickedObject.length){

			document.addEventListener('mousemove', mouseDownMoveCookie);

			document.getElementById("wrapper").onmouseup = (event) => {
				document.getElementById("wrapper").onmouseup = null
				document.removeEventListener('mousemove', mouseDownMoveCookie)
				Body.setStatic(ClickedObject[0], false)
				Body.setMass(ClickedObject[0], 1)
			}
		}
    ...
    var ground = Bodies.rectangle(0, window.innerHeight + 30, window.innerWidth*2, 60, { isStatic: true, render: { fillStyle: 'transparent' }});
    var leftWall = Bodies.rectangle(0-30, 0, 60, window.innerHeight*2, { isStatic: true, render: { fillStyle: 'transparent' }});
    var rightWall = Bodies.rectangle(document.body.clientWidth+30, 0, 60, document.body.clientHeight*3, { isStatic: true, render: { fillStyle: 'transparent' }});
...
Game.Launch=function()
{
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			totalNumCookies += Game.cookiesPs/Game.fps
			
			if (totalTempCookies + 1 < totalNumCookies){
				totalTempCookies = totalNumCookies
				var physicalCookie = Bodies.circle(Common.random(0, document.body.clientWidth), -50, 16,  {friction: 0.00001, restitution: 0.9, density: 0.005, 
					render: {
						sprite: {
				...
				allBodies.push(physicalCookie)
				
				Body.applyForce(physicalCookie, physicalCookie.position, {
                    x:  0, 
                    y: 0
                });
				
				Composite.add(engine.world, [physicalCookie]);
			}
